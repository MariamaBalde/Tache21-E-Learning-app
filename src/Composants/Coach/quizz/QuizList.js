      ...
      {quizzes.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {quizzes.map((quiz) => (
            <div
              key={quiz.id}
              className={`bg-white shadow-md rounded-lg p-4 border ${quiz.archived ? 'border-gray-400' : 'border-blue-600'}`}
            >
              <h3 className="font-semibold text-lg text-blue-600">{quiz.title}</h3>
              ...
              <div className="flex space-x-2 mt-4">
                ...
                <button
                  className={`px-4 py-2 rounded-lg ${quiz.archived ? 'bg-green-600 hover:bg-green-500' : 'bg-red-600 hover:bg-red-500'} text-white`}
