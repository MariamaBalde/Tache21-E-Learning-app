...
const QuizCard = ({ quiz, onToggleArchive }) => {
  ...
  const handleToggleArchive = async () => {
    ...
    await onToggleArchive(quiz.id, quiz.archived); // Action d'archivage/désarchivage
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {/* Jouer au quiz */}
        <Link
          to={`quiz-details/${quiz.id}/play-quiz/${quiz.id}`}
          className="relative group p-2 text-green-600 hover:bg-green-100 rounded-md transition"
        >
        ...
        <Link
          ...
          className="relative group p-2 text-yellow-600 hover:bg-yellow-100 rounded-md transition"
        >
        ...
        <button
          className={`relative group p-2 ${
            quiz.archived ? 'text-green-600' : 'text-red-600'
