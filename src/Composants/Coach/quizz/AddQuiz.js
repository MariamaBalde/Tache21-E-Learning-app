...
import { showSuccess, showError, showInfo } from '../../Shared/ToastifyNotification'; // Importer Toastify
  ...
  const handleAddQuiz = async () => {
    if (quizData.title && quizData.questions.every(q => q.question && q.options.every(o => o))) {
      ...
      try {
        ...
        showSuccess("Quiz ajouté avec succès !"); // Remplacer alert par Toastify
        setQuizData({
          title: "",
          course: "html-css",
        ...
        showError("Une erreur est survenue lors de l'ajout du quiz."); // Remplacer alert par Toastify
        console.error("Erreur lors de l'ajout du quiz : ", error);
      } finally {
        setLoading(false); // Désactiver le loader
      }
    } else {
      showInfo("Veuillez remplir tous les champs."); // Remplacer alert par Toastify
    }
  };
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
        ...
        {showQuestions && quizData.questions.map((q, index) => (
          <div key={index} className="space-y-4 border p-4 rounded-md shadow-sm">
            ...
            {/* Sélecteur pour choisir la bonne réponse */}
            <div className="mt-2">
